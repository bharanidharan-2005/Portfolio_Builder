...
import { API } from "../../api";
import ContentGeneratorTool from '../tools/ContentGeneratorTool';
        ...
        if (selectedTargetSection === "global_bg") {
            ...
            if (onUpdateSectionContent) {
                ...
                setTerminalLogs(prev => [...prev, { type: "success", text: `[SUCCESS] Canvas block backdrop updated!` }]);
                        ...
                        className={`w-full py-3.5 rounded-xl text-xs font-bold transition-all duration-300 shadow-lg flex items-center justify-center gap-2 cursor-pointer active:scale-95 ${isProcessing ? 'bg-blue-900/30 text-blue-300/50 cursor-not-allowed border border-blue-900/20 shadow-none' : 'bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white shadow-blue-900/20 shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)]'
                            }`}
                    >
                        ...
                        >
                            ...
                            {bp.isScore && (
                                <span className={`text-xs font-black px-2.5 py-1.5 rounded-lg border ${currentCompleteness > 80 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-orange-500/10 border-orange-500/20 text-orange-500'}`}>{currentCompleteness}%</span>
                                        ...
                                        className={`w-full flex items-center justify-between p-4 rounded-2xl border text-xs font-bold text-left transition-all duration-300 cursor-pointer group hover:-translate-y-0.5 ${isSelected
            ...
            {paletteTab === "fonts" && (
                <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-3">
                        ...
                        className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg transition-all duration-300 cursor-pointer ${pitchTab === tab
            ...
            {(pitchTab === "email" || pitchTab === "linkedin") && (
                <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
                    ...
                    <button
                        ...
                        className={`w-full py-3.5 rounded-xl text-white text-xs font-bold shadow-lg transition-all duration-300 flex items-center justify-center gap-2 active:scale-95 ${!pitchTarget.trim() || isProcessing ? 'bg-indigo-900/30 text-indigo-300/50 cursor-not-allowed border border-indigo-900/20 shadow-none' : 'bg-gradient-to-b from-indigo-500 to-indigo-600 hover:from-indigo-400 hover:to-indigo-500 shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] shadow-indigo-900/20 cursor-pointer hover:-translate-y-0.5'
                            }`}
                    >
                    ...
                    {generatedPitchText && (
                        ...
                        className={`w-full py-3.5 rounded-xl text-xs font-bold transition-all duration-300 shadow-lg flex items-center justify-center gap-2 cursor-pointer active:scale-95 ${isProcessing ? 'bg-purple-900/30 text-purple-300/50 cursor-not-allowed border border-purple-900/20 shadow-none' : 'bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] hover:-translate-y-0.5 hover:shadow-xl'
                            }`}
                    >
                ...
                <div className="space-y-4 animate-in slide-in-from-right-2 duration-300">
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-xs font-bold transition-all duration-300 shadow-md cursor-pointer active:scale-95 hover:-translate-y-0.5 ${isProcessing
                ...
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-4 rounded-xl text-xs font-bold transition-all duration-300 shadow-lg cursor-pointer active:scale-95 hover:-translate-y-0.5 ${isProcessing
                            ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700 shadow-none'
    ...
    return (
        <div className={`h-full w-full flex flex-col transition-colors duration-500 ${isLight ? 'bg-slate-50/50 backdrop-blur-xl' : 'bg-[#0B0C10]/80 backdrop-blur-2xl'}`}>
            <div className="flex-1 flex flex-col overflow-y-auto p-6 custom-scrollbar">
                {activeTool === "generator" && <ContentGeneratorTool {...toolProps} />}
                {activeTool === "matcher" && <RoleMatcherTool {...toolProps} />}
                {activeTool === "impact" && <ImpactQuantifierTool {...toolProps} />}
                {activeTool === "palette" && <PaletteStudioTool {...toolProps} />}
                {activeTool === "image" && <ImageCustomizerTool {...toolProps} />}
                {activeTool === "seo_pitch" && <SeoPitchTool {...toolProps} />}
                {activeTool === "export" && <CodeExportTool {...toolProps} />}
                {activeTool === "activity_log" && <ActivityLogTool {...toolProps} />}
                {(activeTool === "structure" || (!activeTool && activeTool !== "activity_log")) && <StructureBuilderTool {...toolProps} />}
            </div>
            ...
            <div className={`shrink-0 border-t p-6 flex flex-col gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-10 ${isLight ? 'border-slate-200 bg-white' : 'border-slate-800/80 bg-[#0F111A]'}`}>
                ...
                <div ref={logContainerRef} className={`lg:hidden text-[10px] font-mono h-28 overflow-y-auto space-y-2 custom-scrollbar p-3 rounded-xl border shadow-inner ${isLight ? 'bg-slate-50 border-slate-200 text-slate-600' : 'bg-[#090A0F] border-slate-800 text-slate-400'}`}>
                    ...
                    {terminalLogs && terminalLogs.map((log, i) => (
                        <div key={i} className={`flex items-start gap-2 animate-in fade-in slide-in-from-bottom-1 duration-300 ${log.type === 'system' ? 'opacity-70' : log.type === 'success' ? 'text-emerald-400 font-bold' : log.type === 'error' ? 'text-red-400 font-bold' : log.type === 'user' ? (isLight ? 'text-blue-600 font-bold' : 'text-orange-400 font-bold') : 'italic opacity-50'
                            }`}>
