...
export default function App() {
    // 🧭 STATE MACHINE: LANDING -> NAME -> CREDENTIALS -> VERIFICATION -> PREFERENCES -> WORKSPACE
                ...
                onboardingStep !== 'LANDING' && ( <
...

                    {
                        onboardingStep === 'PREFERENCES' && ( <
                            ...
                            className = "w-full bg-[#a855f7] hover:bg-purple-500 disabled:opacity-40 text-white font-bold py-3 rounded-xl text-xs uppercase tracking-widest transition-all mt-4 shadow-lg shadow-purple-500/20" > Launch Builder < /button> < /
                            div >
                        )
                    } <
                    /div> < /
...

        {
            ...
            setActivePage = {
                (pageName) => {
                    ...
                    if (targetPage && targetPage.sections && targetPage.sections.length > 0) setActiveSectionId(targetPage.sections[0].id);
                }
            }
            onSelectTool = {
