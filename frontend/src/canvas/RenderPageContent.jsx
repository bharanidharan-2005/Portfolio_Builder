...
export default function RenderPageContent({ section, portfolioTheme }) {
    ...
    return ( <
        div className = "w-full" > { /* --------------------------------------------------------- */ } { /* 1. HERO SECTION RENDERER                                  */ } { /* --------------------------------------------------------- */ } {
            currentType === 'hero' && ( <
                div className = "text-center py-8 space-y-4" >
                <
                h1 className = { `text-3xl font-black tracking-tight uppercase ${primaryText}` } > { data.heading || "Your Name" } <
                /h1> <
                p className = { `text-sm max-w-md mx-auto leading-relaxed ${secondaryText}` } > { data.subheading || "Professional Headline" } <
                /p> <
        ...
        { /* --------------------------------------------------------- */ } { /* 2. ABOUT ME NARRATIVE BLOCK                               */ } { /* --------------------------------------------------------- */ } {
            currentType === 'about' && ( <
                ...
                p className = { `text-xs leading-relaxed font-medium ${secondaryText}` } > { data.bio || "Provide a professional summary text context profile." } <
                /p> < /
                div >
            )
        }
            ...
            currentType === 'education' && ( <
                    ...
                    div className = "space-y-3" > {
                        (data.schools || []).map((school, i) => ( <
                                ...
                                <
                                h3 className = { `text-xs font-bold uppercase tracking-wide ${primaryText}` } > { school.institution || "College or Institution" } <
                                /h3> <
                                p className = "text-[11px] font-medium text-purple-400/90" > { school.degree || "Course Certificate Program Major" } <
                                /p> < /
                                div > {
                                    school.years && ( <
                                        span className = "text-[9px] font-mono px-2 py-0.5 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-md shrink-0 whitespace-nowrap" > { school.years } <
                                        /span>
                                    )
                                } <
                                /div> {
    ...
    { /* --------------------------------------------------------- */ } { /* 5. SHOWCASE OF INNOVATIONS (PROJECTS)                     */ } { /* --------------------------------------------------------- */ } {
        currentType === 'projects_grid' && ( <
                ...
                <
                h2 className = "text-[10px] uppercase font-black tracking-widest text-purple-400" > { data.title || "Showcase of Innovations" } <
                /h2> <
                div className = "grid grid-cols-2 gap-4" > {
                    (data.projects || []).map((project, i) => ( <
                            ...
                            <
                            h3 className = { `text-xs font-bold uppercase tracking-wide truncate ${primaryText}` } > { project.title || "Project Name" } <
                            /h3> {
                            project.projectUrl && ( <
                        ...
                        /div> <
                        p className = { `text-[11px] leading-relaxed line-clamp-3 font-medium ${secondaryText}` } > { project.desc || "Operational item details summary context." } <
                        /p> < /
                        div > {
                            project.tags && project.tags.length > 0 && ( <
                                div className = "flex flex-wrap gap-1 pt-1" > {
                                    project.tags.map((tag, j) => ( <
                                        span key = { j }
                                        className = "text-[8px] font-bold px-1.5 py-0.5 bg-slate-800 text-slate-300 border border-slate-700/60 rounded" > { tag } <
                                        /span>
                                    ))
                                } <
                                /div>
...
{ /* --------------------------------------------------------- */ } { /* 6. CONTACT CALL TO ACTION CHANNEL                         */ } { /* --------------------------------------------------------- */ } {
    currentType === 'contact' && ( <
        ...
        p className = { `text-xs max-w-sm mx-auto leading-normal font-medium ${secondaryText}` } > { data.text || "Let's collaborate on production platforms. Reach out directly below." } <
        /p> < /
        div >
    )
} <
